...
> move_only_function 能保存、复制、调用任何**可构造的**可调用对象。
>
...
- 当参数类型为 auto 时，将生成*泛型 lambda*，auto 会被 operator () 函数上的模板参数替代，从而允许接收不同类型。
    也可以使用形参包`[](auto... args) {}`。
...
> 因此在写递归时，比如`f f(int x) {return f(x);}`，编译器想要检查返回值类型，就要一层层展开 f，导致无限循环。
> 这个问题的解决方式有两种：
>
...
与 lock\_guard 类似，但是一个更完整、自由的锁管理器：
...
2. 在持有锁时进行修改。
3. 在 cv 上执行 notify\_one 或 notify\_all。该步可以不持有锁（但是在某些情况下需要，见[这里](https://zh.cppreference.com/w/cpp/thread/condition_variable/notify_one)没看懂）。

等待 cv 的线程需要：

1. 获得同样的 mutex（一般是 std::unique\_lock）。
2. 检查状态是否已更新，如果没有，调用 wait 等待。
...
注意要用 std::localtime 而非 localtime，否则会匹配到 C 的那个。
MSVC 会警告 C 的 localtime 已被弃用，因为它返回的 tm 指针指向的对象，实际上分配在静态存储中，容易导致错误释放。推荐 localtime_s，它要求用户自己提供 tm，并将结果放在里面。
...
各个 clock 都可以通过 now() 返回 time_point，然后通过静态函数`system_clock::to_time_t`转换到 C 风格时间点 time_t。
