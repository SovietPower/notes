...
> 可参考 *Books - C++ - C++17 完全指南*。



**launder**

> https://stackoverflow.com/questions/39429476/reinterpret-cast-char-and-undefined-behavior
>
> 实践中至少对 POD 好像对象生存期是否有效都无所谓，甚至 buffer 不与 T 对齐 asan 也检查不出来？编译器会正确分配边界？C++17 前也不需要考虑 launder（见最下）？
...
必须使用 `launder(reinterpret_cast<T*>(p))`。
...
>
> C++17 前，在数组中 placement new 构造对象会造成存储的重用，使得数组对象生存期结束，但不会导致访问 UB？
...
`__VA_OPT__(sth)` C++20 引入，可以出现在宏替换列表内，当 VA_ARGS 非空时，它会被替换为 sth（可以为空）；当 VA_ARGS 为空时，它为空、不会展开成任何内容。
...
#define PRINT_MACRO(x) #x " = " STRINGIFY(x)
#pragma message(PRINT_MACRO(__LINE__))  // 输出当前行号
```
...
>
> MSVC：https://learn.microsoft.com/en-us/cpp/cpp/attributes?view=msvc-170
...
[[gnu/clang::noinline]] 要求禁止内联某个函数或某次表达式涉及的调用。
应该不是强制性的。
...
> 不是所有平台和架构都支持字节可寻址（地址空间以字节为单位）。在这些平台访问非对齐成员时，编译器能通过结构体的信息（pack）和移位等操作，正确进行读写。但如果将成员绑定到普通引用，就丢失了 pack 信息，编译器会直接访问成员地址（可能并不支持），可能导致崩溃，或发生 trap 影响性能。见 *OS - 内存对齐*。
>
...
忽略类的访问权限 (private, protected)，常用于调试。
