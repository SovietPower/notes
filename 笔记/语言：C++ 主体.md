...
> https://www.zhihu.com/question/6314442778/answer/51861844614
...
> char, (un)signed char, char8_t, char16_t, char32_t, std::byte, wchar_t 是互不相同的类型。
>
...
[POD (plain old data)](https://zh.cppreference.com/w/cpp/language/classes)（*简旧数据类型*）中 plain 指它是一个普通/平凡的类型，old 指与 C 兼容。该概念在 C++20 废弃。
...
比如：使用未初始化的标量类型、数组越界、**有符号整数溢出**、有符号数左移大于等于它的位数（或右操作数为负）（注意移位时需要自行取模，不能左/右移64）（C++20 前）、有符号负数左移（C++20 前）、空指针解引用、非 void 函数执行完成但没有返回值（应声明`__attribute__((noreturn))`）、无副作用的死循环、求值不确定值（如默认初始化的基础类型。除了少数例外）、将浮点值赋给无法表示该值的类型（包括赋 DBL_MAX 给 float）。
...
读写 volatile 泛左值，修改一个对象 (?)，原子同步，修改文件 (IO) 是副作用。
...
> 值初始化会进行零初始化，可能导致比默认初始化更慢，尤其是在使用容器预分配大量元素时。此时可以包装一个默认构造函数不进行任何初始化的类存储。
...
keeper 声明语句有两种解释：用一个临时对象 Timer() 创建和构造 TimeKeeper 对象；声明一个函数，它返回 TimeKeeper，参数为一个函数指针，指向一个返回 Timer 的无参函数。
...
> 严格别名规则是为了效率，对程序安全性的部分放弃，需要程序员注意；原本可以直接强制类型转换取值的，在严格别名优化下需要复制一次。
> 可以关闭严格别名，然后在热点部分用 restrict 等方式做到类似的优化效果。
>
...
> std::memcpy(&n, &d, sizeof d);	// OK. 对这类简单、定长的 copy memcpy 应该会有优化
> n = std::bit_cast<int64_t>(d);	// OK
> ```
>
> 注意正确的双关除了类型兼容，还要保证两者对齐一致。
...
>
> (u)int8_t 一般是 (un)signed char 的别名，所以这两个一般也可以。
...
除了 placement new 外，new 表达式抛出异常时会调用 delete 释放已分配的内存，并不会直接导致内存泄露。

```cpp
unique_ptr<A> p(new A); // new A 抛出异常也 ok
```

内存泄露只出现在内存分配完成、没有释放的情况：

```cpp
f(new A, new A, new A);
```

上面代码中，如果创建某个 A 时异常，则之前创建的 A 对象无法析构、导致泄露。
...
> enum 之间可以直接进行各类比较（包括小于大于），会用其底层值比较。
> 由于各枚举值可以重复，所以两个不同 enum 常量可能相等。
...
> `const vector<T*>` 的元素类型是 T* const，所以可以修改元素指向的对象，但不可以修改元素的指向。
...
> 引用相当于自带顶层 const（不能更改其指向。但引用不能用 cv 修饰）（引用类型上的 const 总是 low-level 的），它的底层 const 与指针类似：const int& 的 const 修饰 int，且在 remove_const 时需要先 remove_ref，因为它是 low-level const。
>
> 引用不能用 cv 修饰。
...
注意，因为 move 的形参是引用，所以对纯右值应用 move 后再将其赋给引用，不会延长其生存期（临时量的生命周期已经与 move 形参一致）：
...
（例外：`"abc"`这样的字面量的生存期与程序一致（即使看起来很像局部变量），所以返回`"abc"`的`const char* f()`是没问题的；但如果它是 string 就不行了）

```cpp
const char* s = (str1 + str2).c_str(); // 悬垂
```
...
[条件运算符](https://zh.cppreference.com/w/cpp/language/operator_other)即三目运算符。

注意对于`E1 ? E2 : E3`，要保证 E2 与 E3 类型和值类别相同，否则非常容易出错，见 *Quiz - 319* 等。

> 例：对于 `int64 x = true ? 221795837984284688ll : Calc(...)`，如果 E3 是 double，那么 E2 这个大数会被转为 double（即使该表达式不可能选择 E3），从而 x 会丢失精度（由于 double 整数精度的问题，值太大了会丢掉后几位，导致 x 总是比预期小一点）。
...
1. 调用构造函数，参数必须是一个通过 new 创建的指针（u_ptr 会自行析构指针，所以如果指向栈内创建的对象将会被析构两次）。
...
> 2. **对同一个 shared\_ptr 的非 const 操作不是线程安全的**（如：reset、swap、operator =），如果要多线程使用，要每个线程都有自己的 shared\_ptr。因此多线程的环境下，函数要使用拷贝而非 const 引用传递 shared\_ptr，否则可能不安全。一个线程改指向，其它线程使用时复制一个 shared\_ptr 也是不安全的（更改指向和改 rc 两者不是原子的）。C++20 起可以使用`atomic<shared_ptr>`，否则只能使用 C++26 会移除的 atomic_load/store。
...
[`xxx_pointer_cast(r)`](https://zh.cppreference.com/w/cpp/memory/shared_ptr/pointer_cast) 对 *shared\_ptr* r 存储的指针调用指定 cast，然后返回包装新指针、但仍使用 r 的引用计数的 shared\_ptr。
前者就是`r.get()`取出指针，后者是用 s_ptr 的别名使用构造函数`shared_ptr(shared_ptr<Y>&& r, element_type* ptr)`构造一个包含与 r 相关的 ptr 指针的 s_ptr，其引用信息与原来的 s_ptr r 关联。
...
> 正因为“绑定引用到纯右值”会触发实质化、产生对象，所以返回 move 的值会破坏 RVO？（引用在哪？）
...
> const 左值引用绑定亡值没有什么效果（因为不允许移动 const 所以不会修改该亡值）；绑定纯右值会延长其生存期。
...
- **亡值** (expiring value, xvalue)：==代表它的资源能够被重新使用的对象或位域的泛左值。==
  ...
  与纯右值非常像，因为往往用于移动、很快就会消失（但是并不保证被移动，还要看实现是否移动该亡值）；但可以是多态的，可以具有不完整类型，也算具名，所以也属于泛左值。
...
> 而且可能影响编译器的 RVO 优化。
>
> 但如果对象不是隐式可移动实体，则不会进行 RVO，那么确实要加 move 进行优化，否则重载决议会选择拷贝构造。
> 主要有几种情况：
>
> - 返回类的成员变量，而这个类对象或该成员不再使用（一般类对象还要持有成员，所以不会移出来）
...
> const 对象无法调用移动构造（不管是 const & 还是 const &&，都会匹配拷贝）。向 move 传 const & 会转为 const && 不会被移动。
...
在上面的 fn 中，x 始终是左值，所以总是会调用第一个 forward，第二个其实是没意义的。
...
因此`static_assert(false)`、`if constexpr(false)`通常会直接编译失败（即使模板使用时不会实例化该语句），而不是在实例化时遇到它才编译失败，所以不能用它去避免某个实例化的发生。可以用一个求值始终为 false 的类型待决表达式（如模板变量）代替 false：
...
void g(T t) {
	if constexpr (std::is_integral_v<T>) {
		...
		static_assert(false); // 会在任何时候都失败，除非使用较新的编译器
		static_assert(dependent_false_v<T>); // ok
		static_assert(!std::is_same_v<T, T>); // ok
	}
}

g(1); // static assertion failed
```

在[constexpr if](https://zh.cppreference.com/w/cpp/language/if#Constexpr_If)里面也有与该场景相关的限制：被舍弃语句不能对所有特化均非良构。

CWG2518 解决了这个问题（不再非良构；见 cppref），但它23年才生效，因此虽然它可应用于 C++11，但只有较新的编译器实现了（如 gcc >= 13）。
...
> 下面代码会输出0：局部静态变量 a 需要进行动态初始化，这会在控制经过对应语句时进行。但在动态初始化之前，依然可以读写 a。此时会读取到它在静态初始化阶段被零初始化后的0。
>
> ```cpp
> int g() { return 1; }
>
...
编译器会为动态初始化生成代码来保证其仅发生一次（类似 call_once）。

```cpp
int f() {
	static int x = X; // X 是 extern 因此会进行动态初始化
	return x++;
}
// 汇编
f():
        push    rbp
        mov     rbp, rsp
        movzx   eax, BYTE PTR guard variable for f()::x[rip]
        test    al, al  ; 判断 guard var 的最低字节是否为 0
        sete    al      ; 如果为 0 设置 al=1；否则 al=0
        test    al, al  ; 判断 al 是否为 0
        je      .L24    ; 为 0 即 guard var 非 0 则跳转
        mov     edi, OFFSET FLAT:guard variable for f()::x
        call    __cxa_guard_acquire ; 获取锁，并返回是否已被其它线程初始化
        test    eax, eax            ; 检查返回值
        setne   al
        test    al, al
        je      .L24                ; 已被初始化，跳转
        mov     eax, DWORD PTR X[rip]       ; 进行动态初始化
        mov     DWORD PTR f()::x[rip], eax
        mov     edi, OFFSET FLAT:guard variable for f()::x  ; guard var 作为第一个参数传入 release
        call    __cxa_guard_release ; 释放锁，同时修改 guard var
.L24:
        mov     eax, DWORD PTR f()::x[rip]
        lea     edx, [rax+1]
        mov     DWORD PTR f()::x[rip], edx
        pop     rbp
        ret
```

cxa_guard 的实现可以看 [cxa_guard.cpp](https://github.com/llvm/llvm-project/blob/main/libcxxabi/src/cxa_guard_impl.h)；接口可以看[这里](https://wiki.osdev.org/C%2B%2B#Local_Static_Variables_(GCC_Only))。
...
C++20 起，引入了 [bit_cast](https://zh.cppreference.com/w/cpp/numeric/bit_cast)，它类似 memcpy，但是允许在编译时完成转换。总之就是告诉编译器这里有另一种类型，并且很可能会将 copy 优化掉。
...
> [这个](https://www.zhihu.com/question/40793741/answer/88412179)讲的很清楚。
...
> 多个弱符号会被随机选择，因此多个 inline 定义会被随机选择。
>
...
> inline 的声明必须在当前翻译单元有定义（可以在 .h 声明、.cpp 定义）。
>
>inline 还会用来定义 *内联命名空间*。

>默认的函数是 extern 的，可能被其它编译单元调用，编译器必须生成符号。但如果函数被标记为 inline 或 static，则编译器可能不会生成它的符号（比如：没有对它取地址/调用等），从而更可能内联它。所以 inline 和 static 还是有隐式、不确定的提示内联作用。
...
>
> 由于 inline 函数的定义（在所有 TU 都要有相同定义），编译器可以将 inline 作为 hint 更激进地内联，比如通过降低内联阈值的方式来优化内联；always_inline 则是直接无视阈值。
> LLVM 会为 inline 函数添加 inlinehint 属性，略微增加该函数的内联阈值，使其更容易被内联，所以它确实会影响 clang 的优化，但不多。
> GCC（可能）会使用函数的伪指令数作为阈值来决定其是否内联，比如对于未标记 inline 的函数，会在伪指令数不超过 30 时进行内联；对 inline 函数，该阈值会调为 400。
...
通过加 u/U/l/L/ll/LL 这些整数字面量后缀（大小写任意，可组合）可以让它至少有 unsigned/long/long long 修饰。

没有负数字面量。-xxx 实际是对字面量 xxx 调用一元减运算符。
C++23 起有 size_t 的整数字面量 z 和 zu（z 是 size_t 的有符号版本。C++ 本身没有 ssize_t，需要通过 make_signed 生成和别名？）。
...
- 命名空间的 using 声明，称为 using 指令 (using-directive)：将另一命名空间的成员引入到当前命名空间或块作用域中。
    注意是引入到当前命名空间，如果在`namespace A`下`using ::B::C`，则 C 作为`A::B::C`可见。
- 类成员的 using 声明 (using-declaration)：可将基类成员引入到派生类的定义中（可以改变私有或保护变量（或私有或保护继承的变量）的访问级别：在 public 下 using 就会将它作为派生类中的公开成员）。
...
>
> 对于`auto *p = x;`和`auto &r = x;`，如果 x 有 const 那么得到的 auto 也有 const。通常写`const auto &`是因为 x 不带 const、希望加上 const 语义。
...
**va_list**（C 中的可变参数实现） / **变长实参** / **可变实参**

> https://blog.csdn.net/q2453303961/article/details/125182787
> https://zh.cppreference.com/w/c/language/variadic
> https://zh.cppreference.com/w/cpp/language/variadic_arguments
> https://zh.cppreference.com/w/cpp/utility/variadic

`va_list, va_start, va_arg, va_end`是一组宏定义。

`va_list`用来定义一组可变参数。它其实就是`char*`或`void*`指针。
通过`f(int n, va_list ap)`定义变量后，需要用`va_start(ap, n)`去初始化 ap，让它指向 n 后面的第一个参数的位置（具体可见上面链接中的宏定义）。
`va_arg(ap, type)`会返回 ap 指针当前指向的 type 类型的值，并增加 sizeof(type) 以指向下一个参数。
`va_end(ap)`清空 va_list，其实就是`ap = (va_list)0;`。

由于这些 va 操作只是宏定义和访问指针，所以编译器其实并不知道参数的类型、不能做类型检查，需要程序自己决定类型，使用时也容易出错。

> 函数调用时，每个变长实参列表内的实参，都会进行名为默认实参提升的隐式转换：整数类型的实参都会经历整数提升，float 类型的实参都隐式转换为 double 类型。
