...
注意，哈希的方式由实现定义，且只保证在一个进程执行内对相同输入有相同输出，不能在进程间协作。

> 原因：https://cplusplus.github.io/LWG/issue2291
...
> 如果 value 的可默认构造且构造代价和拷贝/移动代价不高，直接用 operator [] 查，如果发现为空就更新它即可。
>
...
> 注意，如果 T 的移动构造不是 noexcept 的，vector 在 resize/reserve 时会复制元素而非移动元素！
> 如果移动没加 noexcept，且禁用了拷贝，似乎就只能在 vector 里面存 unique_ptr<T> 了。
...
> - 如果 T 的移动构造没有加 noexcept，则会选择复制构造来保证强异常安全！导致移动失效；大量复制还会导致效率降低与内存增长。
...
对于正常的 STL 容器，调用`operator[]`会返回对应元素的引用 T&。但字节是寻址的基本单位，我们没法获取一个位的地址，所以`vector<bool>`调用`operator[]`会返回一个 proxy reference（代理引用）而非 bool&（所以无法赋给一个 bool\* ）。这不是一个真正的引用，而是一个`std::vector<bool>::reference`类型的返回值。它是一个临时的右值，无法对它取地址。
...
> 但是，string 要能支持转为 const char*，标准规定通过 [c_str](https://zh.cppreference.com/w/cpp/string/basic_string/c_str)、data 返回的指针指向的字符串，其末尾一定要有空终止字符。通常情况下，为了避免拷贝、直接完成转换，string 的实现仍然会在字符串的最后添加 \0（本质还是 Unix/C 字符串要以 \0 结尾带来的问题）（也因此，访问和修改 data[size] 通常不会导致写非法内存）。
...
因为与标准容器类似有 size（不需要以 \0 结尾），所以 string 也可以用作 char buffer，但没有意义不如用 `vector<unsigned char>`。

修改 str.data() 是 ok 的（只要确保不越界和指针不失效），毕竟 str[i] 也会返回对 data[i] 的非 const 引用。
但原则上通过 const_cast 修改 data 还是 UB（编译器可能会针对 const 进行优化，需要注意）。

C++17 起 str.data() 就不再返回 const char\* 了，而是返回 char\*，可以安全修改。
...
[ddb](https://github.com/duckdb/duckdb/blob/main/src/include/duckdb/common/types/string_type.hpp) 的 SSO 实现为：对于长度不超过12的字符串，用4B存储长度，12B存储字符；对于长度超过12的字符串，用4B存储长度，4B存储字符串的前4个字符，8B存储字符串指针。
这种保存4字符前缀的 SSO 可以优化字符串的比较：很多情况下，前4字符就足够比较两个字符串，可以减少一次间接访存。
此外，可以直接将前8B（长度和前4字符）作为一个 uint64_t 读取比较，减少一次比较；在较短时剩余的8B也可以直接作为 uint64_t 比较，而不是一个个字符枚举（当然这个不明显）。
...
测试：查询大小为 N 的`map<String, int>` N 次：

- key 长度很小时 (<=4)，优化后的字符串会比 std::string 快 2.6 倍：https://quick-bench.com/q/5Rd74TtAVtOlfNazLyC_QwZ8mN8
- key 长度较小时 (<=12)，会比 std::string 快 2.2 倍：https://quick-bench.com/q/d7-bjk8WJE9KCLDBixzT01D1weM
- key 为长字符串时，在该测试中会比 std::string 快 1.7 倍：https://quick-bench.com/q/r5L1Ql8UOkYiqZRZQ30Ft7x3-9k
...
struct String1 {
	...
	uint64_t getPrefix1() const {
		return *reinterpret_cast<const uint64_t*>(this);
	}
	uint64_t getPrefix2() const {
		return *reinterpret_cast<const uint64_t*>(reinterpret_cast<const char*>(this) + 8);
	}
	bool operator <(const String1& y) const {
        // 看起来这样比较是负效果
		// auto u = getPrefix1(), v = y.getPrefix1();
		// if (u != v) {
		// 	return u < v;
		// }
		// u = getPrefix2(), v = y.getPrefix2();
		// return u < v;
		if (size != y.size) {
			return size < y.size;
		}
...
struct String2 {
	...
	String2& operator=(const String2& str) {
		size = str.size;
		std::memcpy(prefix, str.prefix, 4);
		data = new char[size];
		std::memcpy(data, str.data, size);
		return *this;
	}
	...
	bool operator <(const String2& y) const {
		auto u = getPrefix(), v = y.getPrefix();
		if (u != v) {
			return u < v;
		}
		for (int i = 0; i < size; ++i) {
...
constexpr bool USE_LONG_STR = false;
constexpr auto SHORT_STR_CONF = 12345ul; // 控制短字符串的长度

static void SSOString(benchmark::State& state) {
  // Code inside this loop is measured repeatedly
  using S = String1;
  std::vector<S> key;
  for (int i = 0; i < N; ++i) {
    // N * 13 位数字构成长字符串
    key.emplace_back(std::to_string(USE_LONG_STR ? i * 9876543210531ul : i * SHORT_STR_CONF));
  }
  std::map<S, int> m;
  for (int i = 0; i < N; ++i) {
    m[key[i]] = i;
...
> https://zhuanlan.zhihu.com/p/25823005592

uniform_int_distribution 是均匀的。

通过取模 `rand()%(R-L+1) + L` 来生成 L~R 之间的随机数显然是不均匀的：由于`RAND_MAX-RAND_MIN+1`并不能整除`R-L+1`，设`RAND_RANGE % (R-L+1) = m`，取模结果在 [0, m) 间的随机数会比 [m, R-L+1] 多 1，所以 L~R 前半段区间的生成概率会比后半段区间略高（除非整除）。
可以拒绝采样：如果 rand 产生的随机数大于`k*(R-L+1)`则重新随机。但在`R-L+1`较小时可能需要随机过多次。

通过映射到 [0, 1] 再转换的随机方式 `rand()/(double)RAND_MAX*(R-L+1) + L` 均匀性更好，但仍不是均匀的：只要随机数样本空间大小`RAND_MAX+1`不是目标样本空间大小`R-L+1`的倍数，这样一一映射就不可能均匀。
