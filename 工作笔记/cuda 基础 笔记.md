...
>
> cuda toolkit 与 driver 版本对应：https://docs.nvidia.com/cuda/cuda-toolkit-release-notes/index.html
> torch 对应版本安装：https://pytorch.org/get-started/previous-versions/，https://zhuanlan.zhihu.com/p/672526561
>
> GPU specification：https://github.com/sunkx109/GPUs-Specs
>
...
> https://github.com/CalvinXKY/BasicCUDA/tree/master
>
> 书籍：
> 《CUDA C编程权威指南》（较旧）
...
>
> 测试：https://leetgpu.com/challenges
...
> warp 是硬件调度的最小单位。
> block 对应 SM，包含许多运算单元，是能完整独立运行和向 gpu 请求调度的最小单元。
...
> block 最大线程数是 32*32=1024，所以对一个 block 的 warp reduce 后写到第一个 warp、再对第一个 warp reduce 就可以 reduce 整个 block 的数据（每次处理 32 需两次）。
...
> block 间的同步：https://zhuanlan.zhihu.com/p/1992329424293627801
...
> A10 有 72 个 SM，T4：40，A100：108，4090：128。

**SMSP**

一个SM被分为多个SMSP，每个SMSP管理固定数量的warp。

SMSP是一个完整的指令执行单元，通常包含一个warp scheduler，一个指令发射单元， 一部分寄存器文件，一组执行单元（如 INT32, FP32, tensor core, ld/st）。每个SMSP在一个 cycle 最多发射1条 warp 指令。
...
线程束 (warp) 是 cuda GPU 调度和执行的基本单元（是 SM 执行的基本单元，准确来说是 SMSP）（是 cuda 软件的概念，但是与硬件直接相关）。
...
> 一个 warp 需要占用一个 SM (SMSP) 运行，多个 warp 需要轮流进入 SM 执行，由 SM 的硬件 warp scheduler 负责调度。
> 所以任意时刻 GPU 上的活跃线程最多只有 SM (SMSP) 数量 * 32 个，不是所有线程都在物理上同时执行（只是逻辑上并行）。
>
...
- Active：被调度到某个 SMSP。
- Eligible：就绪执行下一条指令（未 stalled）。
- TODO

在每个时钟周期，SMSP Warp Scheduler+Dispatch 从其负责的 warp 中选中一个 eligible warp，发射 (issue) 该warp 的一条指令。当某warp的一条指令被发射后，SMSP的对应的 INT32、FP32 等单元就会进行计算。
当没有任何一个 warp 处于 eligible 状态时，该周期被跳过。

当一个 warp 空闲时，SM 就可以调度驻留在该 SM 中的另一个可用 warp。
**在 SM 上保留足够多的活跃 warp 可以隐藏数据加载（访存或有指令依赖）的延时。**活跃 warp 比例可以看 sm occupancy。
...
> 使用 cudaMallocAsync 时会向 cuda runtime 内的一个 memory pool 申请显存，而非直接获取物理显存；但 pool 显存不足时，会申请显存。
> 使用 cudaFreeAsync 时，显存会被放回 pool 但不会真正释放；在同步一个 stream 时，stream 才会通知 pool 释放不需要的显存。
> 见 https://zhuanlan.zhihu.com/p/700224642
>
> 一般情况下（非graph capture期间），cudaMallocAsync/cudaFreeAsync使用的cuda default memory pool，是stream参数所在的device的memory pool（可以用cudaDeviceGetMempool/cudaDeviceSetMempool获取、改变当前的默认memory pool），每个device默认会创建一个memory pool，它也叫default memory pool或者implicit memory pool。
...
使用同步的 cudaMemcpy 时不需要指定流，因为它会在调用时立即执行并完成；使用异步的 cudaMemcpyAsync 需要指定要在哪个 stream 上执行。
...
sm 2.0 起的 gpu 支持**统一虚拟寻址** (UVA) 或**统一虚拟地址空间** (Unified Virtual Address Space)，CPU, GPU 将使用该进程的同一虚拟地址空间进行寻址，通过系统（如 new, malloc）和 cuda 分配的内存都会在该地址空间内。
在统一虚拟地址空间中，CPU 和每个 GPU 都会有一块独立的区间。

- CPU 和 GPU 的页表会记录虚拟地址到物理地址的映射关系，将其分别映射到物理内存和显存。
- 当 GPU 访问某个虚拟地址时，MMU 会查询页表：如果地址属于显存，则直接访问显存；否则通过 PCIe/NVLink 总线触发数据传输访问内存（如零拷贝访问）。
  - 如果属于内存，它必须是`cudaHostAlloc`等分配的 pin memory 以确保物理地址不变。GPU 会在分配时在页表中创建映射。
- 程序不需要区分指针是指向内存还是显存，cuda 会根据地址自动判断。
  也可通过`cudaPointerGetAttributes`手动判断。

**Unified Memory**

sm 3.0 起支持 Unified Memory (UM, managed memory) 或统一内存寻址 (UMA)，UM 是 CPU 和 GPU 都可以访问的内存。（并且可以在任意 GPU 上访问？）
...
cudaMallocFromPoolAsync + cudaMemAllocationTypeManaged、`__managed__`也可创建 um。

> cudaMallocHost、cudaMalloc、H2D、kernel launch、D2H 的标准过程太麻烦了，用 unified memory 来处理内存不用管在哪分配、什么时候拷贝。
...
pinned memory (page-locked host memory, pMem, non-pageable, non-swappable) 是不能被操作系统分页 (swapped out, paged) 的内存。它保存在物理内存中，不会被操作系统的虚拟内存管理机制移动到磁盘的交换空间 (swap space)。
因为 pinned memory 不会被移出内存，其物理内存地址是固定的，因此在进行 DMA（直接内存访问）时，数据传输更快，因为硬件可以直接从已知地址进行操作。

优点：

- device 可直接访问 pMem，以显著提高数据在 host 和 device 之间的传输速度。
  通常在拷贝时，数据需要被拷贝到一个临时的 pMem，然后再传输到 GPU，但如果一开始数据就在 pMem 中，该过程就可省去，从而加速传输。cuda 会记录 pMem 的虚拟地址范围。
  - 使用 pageable 内存与 GPU 传递数据可能很影响性能？（可在 nsys - cuda HW 中确定）

- cuda 不能异步传输非 pMem 的数据，只有保证数据始终在内存中才可异步传输，允许 H2D/D2H/kernel exec 之间并发。
- pMem 可直接映射到 GPU 的地址空间，允许 GPU 零拷贝读取。
...
使用 cudaMallocHost 或 cudaHostAlloc 分配 pMem。
前者是最初的 API，后者是引入其它功能后新加的 API，区别仅在于后者可指定 flag 实现更多功能。
...
**mapped pinned memory** / **zero-copy memory**

`cudaHostAlloc`可指定`cudaHostAllocMapped`来分配零拷贝内存，通过`cudaHostGetDevicePointer`将其映射到 GPU 地址空间后，GPU 可直接访问该 CPU 上的内存无需复制，但访问效率低于 global memory，适用于：

- GPU 没有足够显存存放。
- GPU 只需要加载一次数据，且会做大量计算，此时访存的延迟可被掩盖。
- CPU 需要在 kernel 执行时读写数据，可用于与 kernel 通信？
- 只需要向一块内存写回一次结果时？写回可以是异步的，且即使不使用 zero-copy 也需要向内存进行一次写回。

支持 UVA 后，不指定该 flag pMem 也会映射到相同虚拟地址 (automatic mapping)，且不需要`cudaHostGetDevicePointer`了。pMem 的指针可直接在 GPU 上访问，不需要对一块内存保存多个指针。
即所有 cuda 分配的 pMem 都可同时在 device 和 host 上访问而不需拷贝，所以 pMem 也称为 zero-copy。

因为访问 CPU 内存更慢，所以通过 memory coalescing 减少 zero-copy mem 的内存事务数更重要。
...
stream 内的会严格按序执行；不同 stream 间的任务可以并行执行，以提高 GPU 利用率和 warp occupancy（还需要满足 gpu 未被某个 stream 占满等其它要求）；某个 stream 在做内存传输 (h2d/d2h) 时，另一个 stream 可执行 kernel 计算。
...
每个 thread 都有自己的物理寄存器组，因此切换 warp 时无需恢复上下文，warp 切换开销很低。
...
对于 bank 为 4B 宽度的架构，内存上连续的 128B 由 32 个 bank 上的同一位置的 4B 组成（连续对齐的4字节单元映射到同一 bank），可以并行访问（类似 CPU 的内存，内存带宽的 64B 由 8 个 8B 的 bank 组成）。
...
其中全局内存、局部内存、常量内存都是片下内存 (off-chip)，访问速度一样（较慢），储存在 DRAM/HBM 上，只是访问方式与可见性不同。
...
- 寄存器（文件）：线程能独立访问的资源，用来存储线程的暂存数据。访问最快，但容量较小，SM 的寄存器文件需要划分给所有线程。
...
> cache 大小：
> A10 L1: 128K/SM; L2: 6MB。
> L20 L1: 128K/SM; L2: 96MB。
...
双倍数据速率内存 (DDR, Double Data Rate SDRAM) 和图形双倍数据速率内存 (GDDR, Graphics Double Data Rate SDRAM) 都是基于 SDRAM（同步动态随机存取存储器）技术的内存标准，但设计目标和应用场景有所不同。

DDR 是通用型主内存，是用于 CPU 的最主流的内存类型，见 **计组 - DRAM**。

- 在容量、延迟和成本之间取得平衡：延迟较低，带宽适中；容量高；成本低，功耗低。

GDDR 是专用型显存，为 GPU 设计。

- 最大化内存带宽：通过更高的工作频率和更宽的总线（如 256b, 384b）来实现远超 DDR 的带宽。
- 为了高频率和高带宽，延迟相比 DDR 较高。
- 通常以芯片 (chip) 的形式直接焊接在显卡的 PCB 板上，而不是像 DDR 做成可插拔的内存条。
...
高带宽内存 (HBM, High Bandwidth Memory)：将多个 DRAM 芯片 (die) 堆叠在一起后和 GPU 封装在一起，实现大容量、高位宽、低功耗的 DDR 组合阵列。
...
寄存器是线程私有的，线程间通信必须通过共享内存或全局内存，不能访问其它线程的寄存器，即使寄存器在物理上是共享的（线程可通过 Warp Shuffle 访问同 warp 内的线程，但是是一种特殊、受限、由硬件支持的快速数据交换机制）。
...
每个 SM 有一块物理上独立、专属的寄存器文件。

寄存器会划分为多个独立区域，每个 SMSP 一个（但不是 bank，共享多个 bank）。每个 SMSP 有独立的寄存器记分牌 (scoreboard) 和分配器，通过独立端口访问寄存器文件的不同 bank。

**register file**, **register bank conflict**

> Kepler/Maxwell/Pascal GPU 上寄存器是 4-bank 结构，每个 bank 每个周期只能读或写一个 32b 操作数。如果一个指令需要的 3 个源操作数恰好都落在同一个 bank，就会产生 bank conflict 导致延迟。bank 的分配规则基本是 reg_id % 4。
>
> Volta (SM70) 开始使用双端口寄存器文件 (dual-ported register file)，每个 bank 可以在一个周期内支持 2 读 1 写；且 FFMA/FADD/FMAD 这类三源操作数指令，会采用调度/重命名机制，掩盖潜在的 bank conflict。
> 因此 Volta 后的 gpu 即使三个源都映射到同一 bank，也不会有显著 penalty，大多数情况下硬件会调度/延迟隐藏掉。

以 Turing 和 Volta 为例，每个 processing block 有一个包含 16384 个 32 位元素的寄存器文件 (GF)，线程可见的逻辑寄存器就是该寄存器文件上第 8k 到 8k+7 个元素的不同位置（对 wrap 来说，就是 32\*8k 到 32\*8k+255 个元素的不同位置）。

寄存器文件也是 SRAM，所以像 shared mem 有 bank conflict 可能。
...
> 测内存带宽：https://github.com/NVIDIA/cuda-samples/blob/v12.4/Samples/1_Utilities/bandwidthTest/README.md（make 然后运行即可。结果好像不太稳定）
>
> H2D、D2H、D2D **之间**可以并行，但同类之间需要共享带宽。
