...
2. 创建新分支（如`git checkout -b feat origin/master`）；在修改完 commit 后，通过`git pull --rebase; git push`来合并和提交 master 分支；然后切换到 master 用 pull 更新本地的 master 分支。
...
#### **git add**
`git add`将工作区的文件添加到暂存区。添加到暂存区的文件才可commit到仓库（之前`add`但修改了的文件仍在工作区，需要再`add`进去）。
...
2. 可像`--soft`一样合并多次 commit 记录为一次（但与`--soft`相比需多`git add .`一次）。
...
`git branch -d -r <branchName>`：删除远程某分支。更新需要 push 到服务器上：`git push <name> <branchName>`。
...
`git rebase <branchName>`：设当前分支与目标分支的 LCA 为提交 p，将 p->当前分支 上的提交记录 复制到目标分支的提交之后，然后将当前分支指向最后一个提交。p->当前分支的历史提交依然保留。

与 merge 类似，如果目标分支是当前分支的后继，则只需要将当前分支移动到目标分支指向的提交；rebase 分支后，还要切换到目标分支、将其 rebase 当前分支、以更新到最新提交（或者直接`rebase 当前分支 目标分支`）。

> rebase 就是取出一系列的提交记录，逐个“复制”到另外一个位置。
>
...
`git stash [push]`：存储当前分支下的所有工作区改动。会存在`.git/refs/stash`。
...
- `--index`：恢复时不仅恢复工作区，还恢复暂存区。
- `-k`或`--no-keep-index`：指定不保存暂存区？
- `<file>...`：保存指定文件的改动。
...
`git stash save "msg"`：与 -m 类似？附加信息并保存。
...
refspecs 的格式为`[<src>][:<dest>]`，与 fetch 相同，前者是要拉取的远程仓库分支，后者是要更新然后合并的本地分支。
如果省略本地分支名，则使用与远程分支同名的本地**远程分支**。
...
- `-f`或`--force`：强制推送到远程主机，即使本地的远程分支与实际远程分支有差异。
...
`git push -d <remoteName> <remoteBranch>`：删除远程的指定分支？
...
`git push -u company master`就会向该仓库推送分支，等价于`git push https://git.company.com/123.git master `。
...
`git submodule add [module_path]`：添加子模块。
